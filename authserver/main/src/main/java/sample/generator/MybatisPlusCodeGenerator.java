...
public class MybatisPlusCodeGenerator {
    ...
    public static void main(String[] args) {
        ...
        //模块名称
        String moduleName = "registeredclient";

        //test为自定义的文件生成路径，根据业务进行变更
        //父包名，test为最终生成的包位置，替换成自己的即可
        String parentPackage = "sample.modules";
        ...
        tables.add("oauth2_registered_client");
        System.out.println(System.getProperty("java.class.path"));
        System.out.println("文件生成目录 ： "+ System.getProperty("user.dir") +"/authserver"+ "/src/main/java");
        FastAutoGenerator.create(url, username, password)
                //全局配置
                .globalConfig(builder -> {
                    builder.disableOpenDir()  //禁止打开输出目录
                            .outputDir(System.getProperty("user.dir") +"/authserver"+ "/src/main/java")   //指定输出目录
                            .author(author)   //作者名
...
//                            .fileOverride() //全局覆盖已有文件的配置已失效，已迁移到策略配置中
                ...
                .packageConfig(builder -> {
                    builder.parent(parentPackage)     //父包名
                            ...
                            .pathInfo(Collections.singletonMap(OutputFile.xml, System.getProperty("user.dir")  +"/authserver" + resources + mapperXmlPath + moduleName));//指定xml位置
                })
                //策略配置
